...
namespace Synexens
{
    ...
    //@ param [in/out] bTOFtoRGB 对齐方式 true-以RGB为准对齐  false-以TOF为准对齐
    //@ return 错误码
    extern "C" SYSDK_DLL SYErrorCode GetRGBD(unsigned int nDeviceID, int nSourceDepthWidth, int nSourceDepthHeight, unsigned short* pSourceDepth, int nSourceRGBWidth, int nSourceRGBHeight, unsigned char* pSourceRGB, int nTargetWidth, int nTargetHeight, unsigned short* pTargetDepth, unsigned char* pTargetRGB, bool bTOFtoRGB = true);
    ...
    //@ param [in/out] pFrameData 最后一帧数据
    //@ return 错误码
    extern "C" SYSDK_DLL SYErrorCode GetLastFrameDataSafety(unsigned int nDeviceID, std::shared_ptr<SYFrameData>& pFrameData);
	...
	//@ param [in] bRGB  镜头类型 true-RGB false-TOF
    //@ param [in/out] intrinsics 相机参数
	//@ return 错误码
	extern "C" SYSDK_DLL SYErrorCode GetIntric(unsigned int nDeviceID, SYResolution resolution, bool bRGB, SYIntrinsics& intrinsics);
	...
	//设置帧率
	//@ param [in] nDeviceID 设备ID
	//@ param [in] nFrameRate  帧率
	//@ return 错误码
	extern "C" SYSDK_DLL SYErrorCode SetFrameRate(unsigned int nDeviceID, unsigned int nFrameRate);

	//获取帧率
	//@ param [in] nDeviceID 设备ID
	//@ param [in/out] nFrameRate  帧率
	//@ return 错误码
	extern "C" SYSDK_DLL SYErrorCode GetFrameRate(unsigned int nDeviceID, unsigned int& nFrameRate);

	//获取温度
	//@ param [in] nDeviceID 设备ID
	//@ param [in/out] fltTemperature  温度
	//@ return 错误码
	extern "C" SYSDK_DLL SYErrorCode GetTemperature(unsigned int nDeviceID, float& fltTemperature);

	//查询帧率是否可调
	//@ param [in] nDeviceID 设备ID
	//@ param [in/out] bEnabled  帧率是否可调标志
	//@ return 错误码
	extern "C" SYSDK_DLL SYErrorCode IsFrameRateEnabled(unsigned int nDeviceID, bool& bEnabled);

	//解析配置文件
	//@ param [in] nStringLength 路径字符串长度
	//@ param [in] pFileName  配置文件完整路径
	//@ param [in/out] configuration  配置数据
	//@ return 错误码
	extern "C" SYSDK_DLL SYErrorCode ParseConfiguration(int nStringLength, const char* pFileName, SYConfiguration & configuration);

	//使用配置开流
	//@ param [in] nDeviceID 设备ID
	//@ param [in] configuration  配置数据
	//@ return 错误码
	extern "C" SYSDK_DLL SYErrorCode StartStreamingWithConfiguration(unsigned int nDeviceID, const SYConfiguration & configuration);
	...
	//@ param [in] pFileName  配置文件完整路径
	//@ return 错误码
	extern "C" SYSDK_DLL SYErrorCode ExportConfiguration(unsigned int nDeviceID, int nStringLength, char* pFileName);
