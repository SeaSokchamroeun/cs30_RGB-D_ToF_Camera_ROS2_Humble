...
namespace Synexens
{
	...
	struct SYIntrinsics
	{
		...
		int m_nHeight;
		//tof to rgb rotate
		float m_fltTOF2RGBRotate[9];
		//tof to rgb translation
		float m_fltTOF2RGBTranslation[3];
	};
	...
	//滤波参数
	struct SYFilterParam
	{
		//滤波类型
		SYFilterType m_FilterType = SYFILTERTYPE_NULL;
		//滤波参数数据指针
		float m_paramValue[10];
		//参数数量
		int m_nParamCount = 0;
	};

	//配置参数
	struct SYConfiguration
	{
		//流类型
		SYStreamType m_streamType = SYSTREAMTYPE_NULL;
		//Depth分辨率
		SYResolution m_resolutionDepth = SYRESOLUTION_NULL;
		//RGB分辨率
		SYResolution m_resolutionRGB = SYRESOLUTION_NULL;
		//帧率
		int m_nFrameRate = -1;
		//积分时间
		int m_nIntegralTime = -1;
		//水平镜像标志
		...
		//滤波参数指针
		SYFilterParam m_filterParam[10];
		//滤波数量
